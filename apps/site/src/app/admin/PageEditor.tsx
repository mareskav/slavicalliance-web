...
const PageEditor = ({ slug }: { slug: string }) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/content/pages/${page.slug}`)
      .then(async (response) => {
        if (!response.ok) {
      ...
      .then((payload) => {
        if (!cancelled) setContent(payload.raw)
      })
  ...
  if (loading) {
    return <p className="mt-4 text-sm text-white/58">Načítání souboru...</p>
  }
    ...
    <>
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-white/58">{page.file}</p>
        </div>
        <button
          onClick={handleSave}
